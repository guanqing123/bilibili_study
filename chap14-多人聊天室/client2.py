...
class YsjClient(wx.Frame):
    def __init__(self, client_name):
        ...
        # 创建聊天内容的文本框
        self.chat_text = wx.TextCtrl(panel, size=wx.Size(400, 120), style=wx.TE_MULTILINE)
        ...
        # 给断开按钮绑定事件
        self.Bind(wx.EVT_BUTTON, self.dis_con_server, dis_con_btn)
        # 给重置按钮绑定事件
    ...
    def connect_to_server(self, event):
        ...
        if not self.isConnected:  # 等价于 self.isConnected == False:
            # TCP编程的步骤
            server_host_port = ('127.0.0.1', 9090)
            # 创建socket对象
